"""
Shu-Osher — throughput + GPU memory N-scaling benchmark.
...
def bench_jaxfluids(N, case_tmpl, num_path, tmp_dir):
    ...
    import jax
    run_dir = tmp_dir / f"shu_N{N}"; run_dir.mkdir(exist_ok=True)
    ...
    try:
        ...
        cfl_jxf = 0.5
    n_steps = max(1, int(round(L / (cfl_jxf * (L/N) / A_MAX) * (L / 1.0))))
    n_steps = max(1, int(round(1.8 / (cfl_jxf * (L/N) / A_MAX))))
    tp = N * n_steps / statistics.median(times) / 1e6
    return tp, mem_delta
...
def main():
    ...
    try:
        ...
        tmp_dir   = Path(tempfile.mkdtemp(prefix="tp_shu_"))
    ...
    subtitle = (
        "1-D Euler  ·  Shu-Osher (Mach-3 shock + sin ρ)  ·  WENO5-Z+HLLC+RK3 (fused)  ·  200 steps, median 5 runs\n"
    ...
    fig.tight_layout()
    fig.savefig(OUT / "shu_osher_scaling.png", dpi=150, bbox_inches="tight"); plt.close(fig)
    print(f"Saved -> {OUT/'shu_osher_scaling.png'}")
    ...
    fig.tight_layout()
    fig.savefig(OUT / "shu_osher_memory.png", dpi=150, bbox_inches="tight"); plt.close(fig)
    print(f"Saved -> {OUT/'shu_osher_memory.png'}")
